...
privates.isUserInfoAuth = () =>
	$wxp.getSetting().then(
		res => res.authSetting['scope.userInfo']
	).catch(
		err => Promise.reject(`[from wechat catch]检查用户授权信息失败：${err}`)
	);
...
privates.wxLogin = () =>
	$wxp.login().then(
		...
		err => Promise.reject(`[from wechat catch]执行微信登录失败：${err}`)
	);

// 开发者服务器登录 返回 token
privates.login = () => privates.wxLogin().then(code => {
	if (code) {
		return $wxp.request({
			...
			err => Promise.reject(`[from catch]登录失败：${err}`)
		);
	} else {
...
privates.saveToken = token => $wxp.setStorage({
	key: 'token',
	data: token
}).catch(
	err => Promise.reject(`[from wechat catch]token缓存失败：${err}`)
);
...
privates.getWxUserInfo = () => $wxp.getUserInfo({
	...
	res => res && res.userInfo
).catch(
	err => Promise.reject(`[from wechat catch]获取用户信息失败：${err}`)
);
...
// 执行登录
privates.doLogin = callback => privates.login().then(token => {
	// 前端本地缓存token(代码里缓存)
